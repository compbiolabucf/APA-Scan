...
def Generate_read_coverate_plot(ax, pathin, sample, chrom, geneID, start, end, pos, strand, number):
	...
	caption = ax.fill_between(p,c, color="skyblue", alpha=0.9, label = 'Sample '+str(number))
	...
	for (gene, strand) in geneList:
		if gene == geneID:
			...
			for a_row in ann_tt.itertuples():
				...
				y_limit2 = Generate_read_coverate_plot(ax2, p1_dir, pas_samplenames[0], chrom, geneID, int(start), int(end), int(pos), strand, 1)
				
				ax3 = fig.add_subplot(5,1,3)
				ax3.set_ylabel('Counts')
				y_limit = Generate_read_coverate_plot(ax3, s2_dir, samplenames[1], chrom, geneID, int(start), int(end), int(pos), strand, 2)
	...
	for (gene, strand) in geneList:
		if gene == geneID:
			...
			for a_row in ann_tt.itertuples():
				...
				ax1.set_ylabel('Counts')
				y_limit = Generate_read_coverate_plot(ax1, s1_dir, samplenames[0], chrom, geneID, int(start), int(end), int(pos), strand1, 1)
				
				ax2 = fig.add_subplot(3,1,2)
				ax2.set_xlabel('Position')
				ax2.set_ylabel('Counts')
				y_limit = Generate_read_coverate_plot(ax2, s2_dir, samplenames[1], chrom, geneID, int(start), int(end), int(pos), strand, 2)
