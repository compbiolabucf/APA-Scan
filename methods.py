...
def Quantification(chromosomes, s1_dir, s2_dir, samplenames, filename, output_dir):
	ss = time.time()
	...
	with open(filename, 'r') as h:
		...
		with open(output_dir+"Results.csv",'w') as f:
			...
			writer.writerow(['Chrom', 'Gene Name', 'strand', 'Start', 'End', 'Position', 'p-value', 'Ratio Difference', 'Absolute ratio difference', 'Total Length', 'Total Read count '+samplenames[0], 'Total Read count '+samplenames[1], 'Target Length', 'Target Read Count '+samplenames[0], 'Target Read Count '+samplenames[1], samplenames[0]+': n1', samplenames[0]+': N1', samplenames[1]+': n2', samplenames[1]+': N2'])
	    ...
	    with open(filename,'w') as g:
	    	...
	    	for chrom in chromosomes:
	    		...
	    		for (geneId, strand) in geneList:
	    			rowsOfChr = df.loc[(df[1] == chrom) & (df[11] == geneId)]
	    			targetList = []
	    			for row in rowsOfChr.itertuples():
	    				if str(row[1].strip()).startswith('NM_') or str(row[1].strip()).startswith('NR_'):
		    				exonCount = int(row[8])
		    				exonStartList = row[9].split(',')
		    				exonEndList = row[10].split(',')
		    				txStart = int(row[4])
		    				txEnd = int(row[5])
		    				cdsStart = int(row[6])
		    				cdsEnd = int(row[7])
